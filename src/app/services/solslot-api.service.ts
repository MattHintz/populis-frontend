...
/**
 * HTTP client for the Solslot faucet API — the single remaining
 ...
 *
 * The legacy ``solslot-api.service`` entrypoint re-exports this service
 * during the one-release compatibility window.
 */
@Injectable({ providedIn: 'root' })
export class SolslotApiService {
  ...
  // build time in ``environment.solslotProtocol``, and dynamic
  ...
  /**
   * Recover the public, server-indexed zkPassport receipt for one vault.
   *
   * A 404 is an ordinary "not enrolled" result. Every other transport or
   * server failure is preserved so callers can clear any previously recovered
   * authority and fail closed.
   */
  async getZkPassportEnrollment(
    vaultLauncherId: string,
  ): Promise<ZkPassportEnrollmentRecord | null> {
    try {
      return await firstValueFrom(
        this.http.get<ZkPassportEnrollmentRecord>(
          `${this.base}/zkpassport/enrollments/${encodeURIComponent(vaultLauncherId)}`,
        ),
      );
    } catch (error) {
...
export interface ProtocolInfo {
  ...
  deployment_manifest?: {
    ...
    sgt_tail_hash?: string;
    sgt_full_puzhash?: string;
...
export interface ZkPassportAttestationProof {
  bitpath: number;
  siblings: string[];
}

export interface VaultCredentialReceipt {
  vaultLauncherId: string;
  network: string;
  policyVersion: number;
  identityAttestRoot: string;
  attestationLeafHash: string;
  attestationProof: ZkPassportAttestationProof;
  scopedNullifier?: string | null;
  nullifierType?: number | null;
  serviceScopeHash?: string | null;
  serviceSubscopeHash?: string | null;
...
export interface ZkPassportEnrollmentRecord {
  vaultLauncherId: string;
  network: string;
  policyVersion: number;
